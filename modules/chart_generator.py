...
def generate_single_chart(symbol, df, candlestick_interval, output_dir="charts", projection_points=5):
    """Generate a candlestick chart with future breakout projections."""
    current_date = datetime.now().strftime("%Y-%m-%d")
    dated_folder = os.path.join(output_dir, current_date)
    ...
    try:
        ...
        if "open_time" not in df.columns:
            logger.error(f"Missing 'open_time' column in data for {symbol}. Cannot generate chart.")
            return None

        df['open_time'] = pd.to_datetime(df['open_time'], unit='ms')
        df['open_time'] = df['open_time'].map(mdates.date2num)

        # Calculate dynamic candlestick width
        time_range = df['open_time'].max() - df['open_time'].min()
        candlestick_width = max(0.0005, time_range / len(df) / 2)

        # Create the plot with two subplots (candlesticks and volume)
        fig = plt.figure(figsize=(12, 8), facecolor="black")
        ax_candles, ax_volume = fig.subplots(
            2, 1, sharex=True,
            gridspec_kw={'height_ratios': [4, 1]}  # Adjusted height ratio
        )
        ax_candles.set_facecolor("black")
        ax_volume.set_facecolor("black")

        # Format symbol for title (split by "USD" and add spaces around "/")
        if "USD" in symbol:
            symbol_title = symbol.replace("USD", " / USD")
        else:
            symbol_title = symbol  # Default fallback if "USD" is not found
        ...
        # Add Trend Line and Projected Breakouts
        avg_price = (df['high'] + df['low']) / 2  # Calculate average price
        x_values = df['open_time']
        slope, intercept, _, _, _ = linregress(x_values, avg_price)  # Linear regression
        trendline = slope * x_values + intercept  # Calculate trendline
        ...
        time_step = x_values.iloc[1] - x_values.iloc[0]  # Calculate time difference between candles
        future_times = [last_time + i * time_step for i in range(1, projection_points + 1)]  # Generate future time points
        future_trendline = [slope * t + intercept for t in future_times]

        # Plot Future Trend Line
        ax_candles.plot(
            future_times,
            future_trendline,
            color='orange',
        ...
        # Add Legend for candlestick chart
        ...
        # Plot volume (aligned with candlesticks)
        if "volume" in df:
        ...
        # Adjust y-axis labels to remove scientific notation (e.g., 1e7)
